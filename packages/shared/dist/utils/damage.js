 ...
 */
export function calculateDamageReduction(resist) {
    if (resist >= 0) {
 ...
 */
export function calculateDamage(rawDamage, resist, penetrationFlat = 0, penetrationPercent = 0) {
 ...
 */
export function calculatePhysicalDamage(rawDamage, armor, armorPenFlat = 0, armorPenPercent = 0) {
    return calculateDamage(rawDamage, armor, armorPenFlat, armorPenPercent);
}
/**
 * Calculate magic damage.
 */
export function calculateMagicDamage(rawDamage, magicResist, magicPenFlat = 0, magicPenPercent = 0) {
    return calculateDamage(rawDamage, magicResist, magicPenFlat, magicPenPercent);
}
/**
 * Calculate critical strike damage.
 */
export function calculateCritDamage(baseDamage, critDamageMultiplier = GameConfig.COMBAT.CRIT_DAMAGE_MULTIPLIER) {
    return baseDamage * critDamageMultiplier;
}
/**
 * Roll for critical strike.
 */
export function rollCrit(critChance, random = Math.random) {
    return random() < Math.min(1, Math.max(0, critChance));
}
/**
 * Calculate lifesteal healing.
 */
export function calculateLifesteal(damageDealt, lifestealPercent) {
