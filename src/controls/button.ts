...
class Button
  ...
  init(gameContext: GameContext) {
    ...
    this.pressArea = new Rectangle(
      width + this.styles.padding,
      height + this.styles.padding
    );
  ...
  _render = (gameContext: GameContext) => {
    ...
    canvasRenderingContext.fillText(
      this.text,
      this.position.x + this.styles.padding / 2,
      this.position.y + this.pressArea.h - this.styles.padding / 2
    );
  };
  ...
  private handleCanvasPress = (event: HTMLElementEventMap["click"]) => {
    ...
    if (
      y > this.position.y &&
      y < this.position.y + this.pressArea.h &&
      x > this.position.y &&
      x < this.position.x + this.pressArea.w
    ) {
      this.onPress();
    }
  };
