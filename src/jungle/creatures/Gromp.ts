/**
 * Gromp - A bear-like jungle creature.
 *
 * Uses Bear sprite animations with idle, run, and attack states.
...
// Sprite paths (served from public/assets/sprites/Bear/)
const SPRITE_IDLE = '/assets/sprites/Bear/Bear_Idle.png';
const SPRITE_RUN = '/assets/sprites/Bear/Bear_Run.png';
const SPRITE_ATTACK = '/assets/sprites/Bear/Bear_Attack.png';

// Frame dimensions (all sprites are 256x256 per frame)
const FRAME_WIDTH = 256;
const FRAME_HEIGHT = 256;
const SPRITE_SCALE = 0.35; // Scale down for game
...
/**
 * Stats configuration for Gromp.
 */
export interface GrompStats {
  health: number;
  damage: number;
  attackRange: number;
...
/**
 * Gromp jungle creature (Bear).
 */
export class Gromp extends JungleCreature {
  ...
  constructor(position: Vector, stats: GrompStats) {
    super(position, stats);

    // Create sprite sheets
    // Idle: 8 frames (2048 / 256 = 8)
    ...
    this.idleAnimator.addAnimation(ANIM_IDLE, [0, 1, 2, 3, 4, 5, 6, 7], 0.15, true);

    // Run: loop through all 5 frames
    this.runAnimator.addAnimation(ANIM_RUN, [0, 1, 2, 3, 4], 0.1, true);

    // Attack: play all 9 frames once
    this.attackAnimator.addAnimation(ANIM_ATTACK, [0, 1, 2, 3, 4, 5, 6, 7, 8], 0.08, false);
  ...
  private async loadSprites(): Promise<void> {
    try {
      ...
      console.error('Failed to load Gromp (Bear) sprites:', error);
    }
  }
