...
class LandingOnOppositeSideObjective implements LevelObjective {
  ...
  step(context: GameContext): void {
    ...
    this.isInTheBottomSectionOfThePlanet =
      rocket.position.y > this.landingObjective.target.position.y;
  }

  completed() {
    return (
      this.landingObjective.completed() && this.isInTheBottomSectionOfThePlanet
    );
  }
}
...
function generate() {
  ...
  const mark = new LandingMark(
    new Vector(earth.position.x, earth.position.y - earth.collisionMask.radius),
    earth.collisionMask.radius
  );
  ...
  const level = new Level(
    objects,
    new LandingOnOppositeSideObjective(earth, earth.collisionMask.radius)
  );
