...
export type AffectedByGravitationalConstructor =
  GConstructor<AffectedByGravitationable>;

export function AffectedByGravitationableMixin<
  TBase extends PhysicableConstructor & GConstructor<BaseObject>
>(Base: TBase): AffectedByGravitationalConstructor & TBase {
  return class M extends Base implements AffectedByGravitationable {
    calculateGravitationalAcceleration(context: GameContext) {
      const gravitationals = context.objects.filter(
        (obj) => isGravitationable(obj) && obj.id !== this.id
      ) as (BaseObject & Gravitationable)[];
    ...
    private calculateAccelerationByPlanet(obj: Gravitationable) {
      ...
      if (distanceToPlanet < obj.gravitationalThreshold) {
        const directionalVectorToPlanet = new Vector(
          obj.position.x - this.position.x,
          obj.position.y - this.position.y
        ).normalize();
        // TODO: review this formula below
        directionalVectorToPlanet.scalar(
          obj.gravitationalForce / distanceToPlanet
        );
