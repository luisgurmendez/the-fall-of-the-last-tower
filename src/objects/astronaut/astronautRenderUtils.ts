...
class AstronautRenderUtils {
  static renderAstronaut = (context: GameContext, astronaut: Astronaut) => {
    ...
    RenderUtils.rotateSelf(
      ...
      astronaut.position,
      astronaut.direction.angleTo(new Vector(1, 0))
    );
  ...
  static generateAstronautPixelArt = (
    colors = "eee000222444888355ccc"
  ): HTMLCanvasElement => {
  ...
  static generateAstronautRescuesPixelArts = (
    numOfRescues: number,
    total = 3
  ) => {
    ...
    callTimes(total, (i) => {
      if (i >= numOfRescues) {
        astronautEls.push(
          AstronautRenderUtils.generateAstronautPixelArt(
            "ccc666ccccccccccccccc"
          )
        ); // makes a placeholder of an astronaut
      } else {
