...
const PlanetMixins = AffectedByGravitationableMixin(
  GravitationalMixin(
    PhysicableMixin(CollisionableMixin<Circle>()(PositionableMixin(BaseObject)))
  )
);
...
class Planet extends PlanetMixins {
  ...
  constructor(
    ...
    radius: number,
    hasRing: boolean = RandomUtils.getRandomBoolean(0.3),
  ...
  _render = (context: GameContext) => {
    ...
    if (this.hasRing && !this.isMoon) {
      ...
      canvasRenderingContext.moveTo(
        this.position.x - this.collisionMask.radius,
        this.position.y
      );
