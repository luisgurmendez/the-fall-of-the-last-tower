...
export type ParticlePerimetralPositioning =
  | "top"
  | "bottom"
  | "top-left"
  | "top-right";

export function generateThrusterParticle(
  position: Vector,
  ...
  particle.color = isBlueParticle
    ? new Color(20, RandomUtils.getIntegerInRange(0, 255), 255)
    : new Color(255, RandomUtils.getIntegerInRange(0, 255), 12);
  particle.fade = true;
  const velocityAngleVariation = RandomUtils.getValueInRange(-45, 45);
  particle.velocity = thrustDirection
    .clone()
    .scalar(-1)
...
export function generateSecondaryThrusterParticle(
  position: Vector,
  thrustDirection: Vector
) {
...
export function generateRocketExplotionParticles(position: Vector) {
  return callTimes(50, () => {
    ...
    particle.velocity = new Vector(1, 0)
      .rotate(velocityAngleVariation)
      .scalar(RandomUtils.getNumberWithVariance(10, 20));
    particle.direction = particle.velocity.clone().normalize();
...
export function adjustParticlePositionToMatchRocketPerimeter(
  ...
  if (toPosition === "bottom") {
    ...
    particleOffsetVector = new Vector(
      Math.cos(particleOffsetAngle),
      Math.sin(particleOffsetAngle)
    ).scalar(rocketShape.h / 2);
  }
  ...
  if (toPosition.includes("top")) {
    ...
    particleOffsetVector = new Vector(
      ...
      .scalar(rocketShape.h / 2 - 3)
      .scalar(-1);
  }

  if (toPosition === "top-right") {
    const particleOffsetAngleH = rocketDirection.angleTo(new Vector(0, 1));
    const particleOffsetVectorH = new Vector(
      Math.cos(particleOffsetAngleH),
      Math.sin(particleOffsetAngleH)
    ).scalar(rocketShape.w / 2 - 2);
