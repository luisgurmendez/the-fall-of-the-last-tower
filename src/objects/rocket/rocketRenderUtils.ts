...
class RocketRenderUtils {
  ...
  static renderLaunchDirectional = (context: GameContext) => {
    ...
    canvasRenderingContext.fillText(
      `${(
        (rocket.direction.angleTo(new Vector(0, -1)) * 180) /
        Math.PI
      ).toFixed(2)}º`,
  ...
  static renderRocket = (context: GameContext) => {
    ...
    RenderUtils.rotateSelf(
      canvasRenderingContext,
      rocket.position,
      rocket.direction.angleTo(new Vector(0, -1))
    );
    ...
    canvasRenderingContext.drawImage(
      canvas,
      rocket.position.x - rocket.collisionMask.w / 2,
      rocket.position.y - rocket.collisionMask.h / 2
    );
