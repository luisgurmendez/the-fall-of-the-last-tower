 ...
 * Manages playing animations from a sprite sheet with timing and frame control.
 */

import Vector from '@/physics/vector';
import PixelArtSpriteSheet from './PixelArtSpriteSheet';
import { AnimationDefinition, SpriteRenderOptions } from './types';
...
class PixelArtSpriteAnimator {
   ...
   */
  constructor(spriteSheet: PixelArtSpriteSheet, idleFrame = 0) {
  ...
  playAnimation(
    ...
    if (!animation) {
      console.warn(`PixelArtSpriteAnimator: Animation '${name}' not found`);
      return;
    }
